...
import (
	...
	"github.com/paxeer/paxeer/v20/crypto/ethsecp256k1"
...
const (
	// MainnetChainID defines the HyperPaxeer EIP155 chain ID for mainnet
	MainnetChainID = "hyperpax_125"
	// TestnetChainID defines the HyperPaxeer EIP155 chain ID for testnet
	TestnetChainID = "hyperpax_124"
	// TestingChainID defines the HyperPaxeer EIP155 chain ID for testing purposes
	// like the local node.
	TestingChainID = "hyperpax_126"
	// SixDecChainID defines the HyperPaxeer EIP155 chain ID with 6 decimals precision
	SixDecChainID = "hyperpaxix_124"
	// ICSChainID defines the HyperPaxeer EIP155 chain ID with IBC uatom as denom with 6 decimals precision
	ICSChainID = "hyperpaxics_124"
)
...
// IsMainnet returns true if the chain-id has the HyperPaxeer mainnet EIP155 chain prefix.
func IsMainnet(chainID string) bool {
	return strings.HasPrefix(chainID, MainnetChainID)
}

// IsTestnet returns true if the chain-id has the HyperPaxeer testnet EIP155 chain prefix.
...
// GetHyperPaxeerAddressFromBech32 returns the sdk.Account address of given address,
// while also changing bech32 human readable prefix (HRP) to the value set on
// the global sdk.Config (eg: `pax`).
