...
func (suite *LedgerTestSuite) TestPaxeerLedgerDerivation() {
	testCases := []struct {
		name     string
		mockFunc func()
		expPass  bool
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			derivationFunc := ledger.PaxeerLedgerDerivation()
...
func (suite *LedgerTestSuite) TestGetAddressPubKeySECP256K1() {
	...
	addr := crypto.PubkeyToAddress(privKey.PublicKey)
	expAddr, err := sdk.Bech32ifyAddressBytes("paxeer", common.HexToAddress(addr.String()).Bytes())
	suite.Require().NoError(err)
