	...
	testCases := []struct {
	...
	}{
		{
			...
			banktypes.Metadata{
				...
				Name:        "Paxeer",
				Symbol:      "PAXEER",
				Description: "EVM, staking and governance denom of Paxeer",
				DenomUnits: []*banktypes.DenomUnit{
					{
						Denom:    "ahpx",
						Exponent: 0,
						Aliases:  []string{"atto paxeer"},
					},
					{
						Denom:    "paxeer",
						Exponent: 18,
...
func TestEqualAliases(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			[]string{"atto paxeer", "ahpx"},
			[]string{"ahpx", "atto paxeer"},
			false,
		},
		{
			"same values, sorted",
