...
func (tx TransactionLogs) Validate() error {
	if paxtypes.IsEmptyHash(tx.Hash) {
...
func (log *Log) Validate() error {
	if err := paxtypes.ValidateAddress(log.Address); err != nil {
		return fmt.Errorf("invalid log address %w", err)
	}
	if paxtypes.IsEmptyHash(log.BlockHash) {
		return fmt.Errorf("block hash cannot be the empty %s", log.BlockHash)
	}
	if log.BlockNumber == 0 {
		return errors.New("block number cannot be zero")
	}
	if paxtypes.IsEmptyHash(log.TxHash) {
		return fmt.Errorf("tx hash cannot be the empty %s", log.TxHash)
	}
	return nil
}

// ToEthereum returns the Ethereum type Log from an HyperPaxeer proto compatible Log.
...
// LogsToEthereum casts the HyperPaxeer logs to a slice of Ethereum Logs.
