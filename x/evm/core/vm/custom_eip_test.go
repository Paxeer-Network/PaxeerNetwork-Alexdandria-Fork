	...
	testCases := []struct {
		...
		},
		{
			...
			map[string]func(*JumpTable){
				"paxeer_0": func(jt *JumpTable) {},
			},
			true,
			"",
			func() {
				eips := GetActivatorsEipNames()
				require.ElementsMatch(t, append(eips_snapshot, "paxeer_0"), eips, "expected eips number to be equal")
			},
		},
		{
			"success - multiple new activators",
			map[string]func(*JumpTable){
				"paxeer_1": func(jt *JumpTable) {},
				"paxeer_2": func(jt *JumpTable) {},
			},
			true,
			"",
			func() {
				...
				require.ElementsMatch(t, append(eips_snapshot, "paxeer_0", "paxeer_1", "paxeer_2"), eips, "expected eips number to be equal")
			},
		},
		{
