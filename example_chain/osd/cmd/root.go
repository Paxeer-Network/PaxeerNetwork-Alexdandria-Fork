...
import (
	...
	paxcmd "github.com/Paxeer-Network/hyperpaxeer-os/client"
	paxcmdconfig "github.com/Paxeer-Network/hyperpaxeer-os/cmd/config"
	paxkeyring "github.com/Paxeer-Network/hyperpaxeer-os/crypto/keyring"
	"github.com/Paxeer-Network/hyperpaxeer-os/example_chain"
	cmdcfg "github.com/Paxeer-Network/hyperpaxeer-os/example_chain/osd/config"
	paxserver "github.com/Paxeer-Network/hyperpaxeer-os/server"
...
func NewRootCmd() *cobra.Command {
	...
	tempApp := example_chain.NewExampleApp(
		...
		true,
		emptyAppOptions{},
		example_chain.PaxeerAppOptions,
	)
	...
	initClientCtx := client.Context{}.
		...
		// HyperPaxeer specific setup
		WithKeyringOptions(paxkeyring.Option()).
		WithLedgerHasProtobuf(true)

	rootCmd := &cobra.Command{
		Use:   "osd",
		Short: "exemplary HyperPaxeer app",
...
func InitAppConfig(denom string) (string, interface{}) {
	type CustomAppConfig struct {
		...
		EVM     paxserverconfig.EVMConfig
		JSONRPC paxserverconfig.JSONRPCConfig
		TLS     paxserverconfig.TLSConfig
	}
	...
	customAppConfig := CustomAppConfig{
		Config:  *srvCfg,
		EVM:     *paxserverconfig.DefaultEVMConfig(),
		JSONRPC: *paxserverconfig.DefaultJSONRPCConfig(),
		TLS:     *paxserverconfig.DefaultTLSConfig(),
	}

	customAppTemplate := serverconfig.DefaultConfigTemplate +
		paxserverconfig.DefaultEVMConfigTemplate
...
func initRootCmd(rootCmd *cobra.Command, osApp *example_chain.ExampleChain) {
	...
	// add HyperPaxeer' flavored TM commands to start server, etc.
	paxserver.AddCommands(
		rootCmd,
		paxserver.NewDefaultStartOptions(newApp, example_chain.DefaultNodeHome),
		appExport,
		addModuleInitFlags,
	)

	// add HyperPaxeer key commands
	rootCmd.AddCommand(
		paxcmd.KeyCommands(example_chain.DefaultNodeHome, true),
	)
...
func newApp(
	...
	if chainID == "" {
		chainID, err = paxcmdconfig.GetChainIDFromHome(homeDir)
		if err != nil {
			panic(err)
		}
	}
	...
	// Set up the required mempool and ABCI proposal handlers for HyperPaxeer
...
func appExport(
	...
	if height != -1 {
		exampleApp = example_chain.NewExampleApp(logger, db, traceStore, false, appOpts, example_chain.PaxeerAppOptions)

		if err := exampleApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		exampleApp = example_chain.NewExampleApp(logger, db, traceStore, true, appOpts, example_chain.PaxeerAppOptions)
	}
