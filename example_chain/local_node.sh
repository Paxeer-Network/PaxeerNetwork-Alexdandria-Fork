...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	jq '.app_state.erc20.token_pairs=[{contract_owner:1,erc20_address:"0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",denom:"ahpx",enabled:true}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	osd genesis add-genesis-account "$VAL_KEY" 100000000000000000000000000ahpx --keyring-backend "$KEYRING" --home "$HOMEDIR"
	osd genesis add-genesis-account "$USER1_KEY" 1000000000000000000000ahpx --keyring-backend "$KEYRING" --home "$HOMEDIR"
	osd genesis add-genesis-account "$USER2_KEY" 1000000000000000000000ahpx --keyring-backend "$KEYRING" --home "$HOMEDIR"
	osd genesis add-genesis-account "$USER3_KEY" 1000000000000000000000ahpx --keyring-backend "$KEYRING" --home "$HOMEDIR"
	osd genesis add-genesis-account "$USER4_KEY" 1000000000000000000000ahpx --keyring-backend "$KEYRING" --home "$HOMEDIR"

	# Sign genesis transaction
	osd genesis gentx "$VAL_KEY" 1000000000000000000000ahpx --gas-prices ${BASEFEE}ahpx --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$HOMEDIR"
