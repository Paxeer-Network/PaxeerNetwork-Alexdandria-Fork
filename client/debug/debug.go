...
import (
	...
	paxeer "github.com/Paxeer-Network/hyperpaxeer-os/types"
...
func Cmd() *cobra.Command {
	...
	cmd.AddCommand(
		...
		// HyperPaxeer adjusted debug commands
		PubkeyCmd(),
		AddrCmd(),
		RawBytesCmd(),
		LegacyEIP712Cmd(),
...
func AddrCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf(
			`$ %s debug addr pax1qqqqhe5pnaq5qq39wqkn957aydnrm45sdn8583
$ %s debug addr 0x00000Be6819f41400225702D32d3dd23663Dd690 --prefix paxeer`, version.AppName, version.AppName),
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			addrString := args[0]
			switch {
	...
	cmd.Flags().String(flagPrefix, "", "Bech32 encoded account prefix, for example paxeer, paxeervaloper")
	return cmd
}
...
func LegacyEIP712Cmd() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(`$ %s debug legacy-eip712 tx.json --chain-id hyperpaxd_9000-1`, version.AppName),
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			chainID, err := paxeer.ParseChainID(clientCtx.ChainID)
			if err != nil {
