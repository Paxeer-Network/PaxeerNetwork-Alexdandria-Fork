...
CHAINID="${CHAIN_ID:-hyperpax_126-1}"
...
# Set moniker and chain-id for Paxeer (Moniker can be anything, chain-id must be an integer)
osd init "$MONIKER" --chain-id "$CHAINID" --home "$CHAINDIR"

# Change parameter token denominations to ahpx
jq '.app_state["staking"]["params"]["bond_denom"]="ahpx"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="ahpx"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state["gov"]["params"]["min_deposit"][0]["denom"]="ahpx"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state["evm"]["params"]["evm_denom"]="ahpx"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state["mint"]["params"]["mint_denom"]="ahpx"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
...
osd genesis add-genesis-account "$(osd keys show "$VAL_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 100000000000000000000000000ahpx --keyring-backend "$KEYRING" --home "$CHAINDIR"
osd genesis add-genesis-account "$(osd keys show "$USER1_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 1000000000000000000000ahpx --keyring-backend "$KEYRING" --home "$CHAINDIR"
osd genesis add-genesis-account "$(osd keys show "$USER2_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 1000000000000000000000ahpx --keyring-backend "$KEYRING" --home "$CHAINDIR"
osd genesis add-genesis-account "$(osd keys show "$USER3_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 1000000000000000000000ahpx --keyring-backend "$KEYRING" --home "$CHAINDIR"
osd genesis add-genesis-account "$(osd keys show "$USER4_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 1000000000000000000000ahpx --keyring-backend "$KEYRING" --home "$CHAINDIR"
...
osd genesis gentx "$VAL_KEY" 1000000000000000000000ahpx --gas-prices ${BASEFEE}ahpx --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$CHAINDIR"
