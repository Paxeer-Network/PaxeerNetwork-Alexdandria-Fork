...
function checkTestEnv () {
  const argv = yargs(hideBin(process.argv))
    ...
    .example('$0 --network paxeer', 'run all tests using paxeer network')
    .example(
      '$0 --network paxeer --allowTests=test1,test2',
      'run only test1 and test2 using paxeer network'
    )
    .help('h')
    .alias('h', 'help')
    .describe('network', 'set which network to use: ganache|paxeer')
    .describe(
      'batch',
  ...
  if (!argv.network) {
    ...
    if (argv.network !== 'paxeer' && argv.network !== 'ganache') {
      panic('network is invalid. Must be ganache or paxeer')
    } else {
...
function loadTests (runConfig) {
  ...
  fs.readdirSync(path.join(__dirname, 'suites')).forEach((dirname) => {
    ...
    try {
      ...
      const needScripts = ['test-ganache', 'test-paxeer']
...
function performTestSuite ({ testName, network }) {
  const cmd = network === 'ganache' ? 'test-ganache' : 'test-paxeer'
...
function setupNetwork ({ runConfig, timeout }) {
  if (runConfig.network !== 'paxeer') {
    // no need to start ganache. Truffle will start it
    return
  }

  // Spawn the paxeer process
