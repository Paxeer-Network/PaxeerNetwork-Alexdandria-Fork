...
import (
	...
	paxeeribc "github.com/Paxeer-Network/hyperpaxeer-os/ibc"
	precompilestestutil "github.com/Paxeer-Network/hyperpaxeer-os/precompiles/testutil"
...
func init() {
	cfg := sdk.GetConfig()
	cfg.SetBech32PrefixForAccount("paxeer", "paxeerpub")
}
...
func TestGetTransferSenderRecipient(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			data: transfertypes.FungibleTokenPacketData{
				Sender:   "cosmos1",
				Receiver: "pax1x2w87cvt5mqjncav4lxy8yfreynn273xn5335v",
				Amount:   "123456",
			},
			expSender:    "",
			expRecipient: "",
		...
		{
			name: "valid - cosmos sender, paxeer recipient",
			data: transfertypes.FungibleTokenPacketData{
				Sender:   "cosmos1qql8ag4cluz6r4dz28p3w00dnc9w8ueulg2gmc",
				Receiver: "pax1x2w87cvt5mqjncav4lxy8yfreynn273xn5335v",
				Amount:   "123456",
			},
			expSender:    "pax1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			expRecipient: "pax1x2w87cvt5mqjncav4lxy8yfreynn273xn5335v",
			expError:     false,
		},
		{
			name: "valid - paxeer sender, cosmos recipient",
			data: transfertypes.FungibleTokenPacketData{
				Sender:   "pax1x2w87cvt5mqjncav4lxy8yfreynn273xn5335v",
				Receiver: "cosmos1qql8ag4cluz6r4dz28p3w00dnc9w8ueulg2gmc",
				Amount:   "123456",
			},
			expSender:    "pax1x2w87cvt5mqjncav4lxy8yfreynn273xn5335v",
			expRecipient: "pax1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			expError:     false,
		},
		{
			name: "valid - osmosis sender, paxeer recipient",
			data: transfertypes.FungibleTokenPacketData{
	...
	for _, tc := range testCases {
		sender, recipient, _, _, err := paxeeribc.GetTransferSenderRecipient(tc.data)
		if tc.expError {
...
func TestGetTransferAmount(t *testing.T) {
	...
	for _, tc := range testCases {
		amt, err := paxeeribc.GetTransferAmount(tc.packet)
		if tc.expError {
			require.Error(t, err, tc.name)
		} else {
			require.NoError(t, err, tc.name)
...
func TestGetReceivedCoin(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			"transfer",
			"channel-0",
			"transfer/channel-0/ahpx",
			"10",
			sdk.Coin{Denom: "ahpx", Amount: math.NewInt(10)},
		},
		{
	...
	for _, tc := range testCases {
		coin := paxeeribc.GetReceivedCoin(tc.srcPort, tc.srcChannel, tc.dstPort, tc.dstChannel, tc.rawDenom, tc.rawAmount)
...
func TestGetSentCoin(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			"get unwrapped ahpx coin",
			baseDenom,
			"10",
			sdk.Coin{Denom: baseDenom, Amount: math.NewInt(10)},
		},
		{
			"get ibc wrapped ahpx coin",
			"transfer/channel-0/ahpx",
			"10",
			sdk.Coin{Denom: precompilestestutil.ApaxeerIbcdenom, Amount: math.NewInt(10)},
		},
		{
	...
	for _, tc := range testCases {
		coin := paxeeribc.GetSentCoin(tc.rawDenom, tc.rawAmount)
		require.Equal(t, tc.expCoin, coin)
	}
}
...
func TestDeriveDecimalsFromDenom(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expDec:    0,
			expFail:   true,
			expErrMsg: "Should be either micro ('u[...]') or atto ('a[...]'); got: \"npaxeer\"",
		},
		{
	...
	for _, tc := range testCases {
		dec, err := paxeeribc.DeriveDecimalsFromDenom(tc.baseDenom)
		if tc.expFail {
...
func TestIsBaseDenomFromSourceChain(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := paxeeribc.IsBaseDenomFromSourceChain(tt.denom)
