...
import (
	...
	"github.com/Paxeer-Network/hyperpaxeer-os/crypto/hd"
	exampleapp "github.com/Paxeer-Network/hyperpaxeer-os/example_chain"
	chaincmd "github.com/Paxeer-Network/hyperpaxeer-os/example_chain/osd/cmd"
	"github.com/Paxeer-Network/hyperpaxeer-os/server/config"
...
type Config struct {
	...
	GenesisState      paxtypes.GenesisState // custom gensis state to provide
...
func DefaultConfig() Config {
	chainID := fmt.Sprintf("paxeer_%d-1", cmtrand.Int63n(9999999999999)+1)
	dir, err := os.MkdirTemp("", "simapp")
	if err != nil {
		panic(fmt.Sprintf("failed creating temporary directory: %v", err))
	}
	defer os.RemoveAll(dir)
	tempApp := exampleapp.NewExampleApp(log.NewNopLogger(), dbm.NewMemDB(), nil, true, simutils.NewAppOptionsWithFlagHome(dir), exampleapp.PaxeerAppOptions, baseapp.SetChainID(chainID))
	...
	cfg := Config{
		...
		AccountTokens:     sdk.TokensFromConsensusPower(1000000000000000000, paxtypes.AttoPowerReduction),
		StakingTokens:     sdk.TokensFromConsensusPower(500000000000000000, paxtypes.AttoPowerReduction),
		BondedTokens:      sdk.TokensFromConsensusPower(100000000000000000, paxtypes.AttoPowerReduction),
...
// NewAppConstructor returns a new Paxeer AppConstructor
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	if !paxtypes.IsValidChainID(cfg.ChainID) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "hyperpaxd")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "paxeercli")
