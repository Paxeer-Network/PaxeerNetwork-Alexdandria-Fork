...
// RegisterPaxeerERC20Coins uses the UnitNetwork to register the paxeer token as an
...
func RegisterPaxeerERC20Coins(
	...
	paxeerMetadata, found := network.App.BankKeeper.GetDenomMetaData(network.GetContext(), bondDenom)
	if !found {
		return erc20types.TokenPair{}, fmt.Errorf("expected paxeer denom metadata")
	}

	_, err = network.App.Erc20Keeper.RegisterERC20Extension(network.GetContext(), paxeerMetadata.Base)
	if err != nil {
		return erc20types.TokenPair{}, err
	}

	paxeerDenomID := network.App.Erc20Keeper.GetDenomMap(network.GetContext(), bondDenom)
	tokenPair, ok := network.App.Erc20Keeper.GetTokenPair(network.GetContext(), paxeerDenomID)
	if !ok {
		return erc20types.TokenPair{}, fmt.Errorf("expected paxeer erc20 token pair")
	}
