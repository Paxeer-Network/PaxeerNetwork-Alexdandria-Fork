...
// the WHPX and the provided denoms.
...
func CreateGenesisWithTokenPairs(keyring testkeyring.Keyring, denoms ...string) network.CustomGenesisState {
	...
	for _, genesisAccount := range genesisAccounts {
		...
		// e.g. https://github.com/Paxeer-Network/cosmos-sdk/blob/v0.47.5-paxeer.2/x/auth/keeper/keeper_test.go#L193-L223
	...
	// with the WHPX (default is mainnet) and 'xmpl' tokens in the erc20 params
	erc20GenesisState := exampleapp.NewErc20GenesisState()
	erc20GenesisState.TokenPairs = tokenPairs
	erc20GenesisState.Params.NativePrecompiles = []string{constants.WHPXContractMainnet}
