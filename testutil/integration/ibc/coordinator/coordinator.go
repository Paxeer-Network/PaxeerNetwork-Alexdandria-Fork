...
import (
	...
	paxeeribc "github.com/Paxeer-Network/hyperpaxeer-os/ibc/testing"
	"github.com/Paxeer-Network/hyperpaxeer-os/testutil/integration/common/network"
	ibcchain "github.com/Paxeer-Network/hyperpaxeer-os/testutil/integration/ibc/chain"
)
...
type Coordinator interface {
	...
	GetPath(a, b string) *paxeeribc.Path
...
// GetPath returns the transfer path for the chain ids 'a' and 'b'
func (c *IntegrationCoordinator) GetPath(a, b string) *paxeeribc.Path {
	chainA := c.coord.GetChain(a)
	chainB := c.coord.GetChain(b)

	return paxeeribc.NewTransferPath(chainA, chainB)
}
...
func (c *IntegrationCoordinator) Setup(a, b string) IBCConnection {
	path := c.GetPath(a, b)
	paxeeribc.SetupPath(c.coord, path)
