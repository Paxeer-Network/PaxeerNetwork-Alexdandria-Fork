...
- Use the Paxeer security email ([security@paxeer.app](mailto:security@paxeer.app)) to disclose all vulnerabilities,
...
Paxeer uses the following disclosure process:
...
2. The team determines the vulnerability’s potential impact on Paxeer.
...
- For `High` and `Critical`:
    1. Patches are prepared for supported releases of Paxeer in a
    ...
    4. The nodes and validators update their Paxeer and Ethermint dependencies to use these releases.
    5. A week (or less) after the security vulnerability has been patched on Paxeer,
...
including but not limited to Paxeer,
as secure as possible.

### Payment Process

The payment process will be executed according to Paxeer SAFU for `Critical` and `High` severity vulnerabilities.
Payouts can only be executed in accordance and under supervision of the Paxeer Operations team and only once the
...
KYC/KYB result
(Accepted or Rejected) to the Paxeer Core Team. The Paxeer Core team does not have access to any of the information
...
The team commits to releasing security patch releases for the latest release that Paxeer is running.

If HyperPaxeer licensees are running older versions, we encourage them to upgrade at the earliest opportunity
...
are welcomed to backport security patches to older versions for their own use, the Paxeer team reserves
...
The Paxeer Security Team is constantly being monitored.
If you need to reach out to the team directly,
please reach out via email: [security@paxeer.app](mailto:security@paxeer.app)
