...
- Use the Paxeer Network security email ([security@paxeer.app](mailto:security@paxeer.app)) to disclose all vulnerabilities,
...
Paxeer Network uses the following disclosure process:
...
2. The team determines the vulnerability’s potential impact on Paxeer Network.
...
- For `High` and `Critical`:
    1. Patches are prepared for supported releases of Paxeer Network in a
    ...
    4. The nodes and validators update their Paxeer Network and Ethermint dependencies to use these releases.
    5. A week (or less) after the security vulnerability has been patched on Paxeer Network,
...
including but not limited to Paxeer Network,
as secure as possible.

### Payment Process

The payment process will be executed according to Paxeer Network SAFU for `Critical` and `High` severity vulnerabilities.
Payouts can only be executed in accordance and under supervision of the Paxeer Network Operations team and only once the
...
The team commits to releasing security patch releases for the latest release that Paxeer Network is running.
...
are welcomed to backport security patches to older versions for their own use, the Paxeer Network team reserves
...
The Paxeer Network Security Team is constantly being monitored.
