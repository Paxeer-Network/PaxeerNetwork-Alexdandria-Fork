...
import (
	...
	"github.com/Paxeer-Network/hyperpaxeer-os/precompiles/bank"
	"github.com/Paxeer-Network/hyperpaxeer-os/precompiles/bank/testdata"
...
type IntegrationTestSuite struct {
	...
	paxeerAddr, xmplAddr   common.Address
...
func (is *IntegrationTestSuite) SetupTest() {
	...
	is.paxeerAddr = common.HexToAddress(tokenPair.Erc20Address)
...
var _ = Describe("Bank Extension -", func() {
	var (
		...
		paxeerTotalSupply, _ = new(big.Int).SetString("200003000000000000000000", 10)
	...
	Context("Direct precompile queries", func() {
		...
		Context("totalSupply query", func() {
			It("should return the correct total supply", func() {
				...
				Expect(balances[0].Amount.String()).To(Equal(paxeerTotalSupply.String()))
				Expect(balances[1].Amount.String()).To(Equal(xmplTotalSupply.String()))
			})
		})

		Context("supplyOf query", func() {
			It("should return the supply of Paxeer", func() {
				queryArgs, supplyArgs := getTxAndCallArgs(directCall, contractData, bank.SupplyOfMethod, is.paxeerAddr)
				_, ethRes, err := is.factory.CallContractAndCheckLogs(sender.Priv, queryArgs, supplyArgs, passCheck)
				Expect(err).ToNot(HaveOccurred(), "unexpected result calling contract")
				...
				Expect(out[0].(*big.Int).String()).To(Equal(paxeerTotalSupply.String()))
			})
	...
	Context("Calls from a contract", func() {
		...
		Context("supplyOf query", func() {
			It("should return the supply of Paxeer", func() {
				queryArgs, supplyArgs := getTxAndCallArgs(contractCall, contractData, SupplyOfFunction, is.paxeerAddr)
				_, ethRes, err := is.factory.CallContractAndCheckLogs(sender.Priv, queryArgs, supplyArgs, passCheck)
				Expect(err).ToNot(HaveOccurred(), "unexpected result calling contract")
