...
import (
	...
	paxutiltx "github.com/Paxeer-Network/hyperpaxeer-os/testutil/tx"
)

func (s *PrecompileTestSuite) TestBalances() {
	var ctx sdk.Context
	// setup test in order to have s.precompile, s.paxeerAddr and s.xmplAddr defined
	s.SetupTest()
	...
	testcases := []struct {
		...
		expPass     bool
		errContains string
		expBalances func(paxeerAddr, xmplAddr common.Address) []bank.Balance
	}{
		{
		...
		},
		{
			...
			true,
			"",
			func(paxeerAddr, xmplAddr common.Address) []bank.Balance {
				return []bank.Balance{
					{
						ContractAddress: paxeerAddr,
		...
		{
			"pass - PAXEER and XMPL balances present - mint extra XMPL",
			func() []interface{} {
	...
	for _, tc := range testcases {
		s.Run(tc.name, func() {
			...
			if tc.expPass {
				...
				s.Require().NoError(err)
				s.Require().Equal(tc.expBalances(s.paxeerAddr, s.xmplAddr), balances)
			} else {
...
func (s *PrecompileTestSuite) TestTotalSupply() {
	...
	s.Require().NoError(err)
	paxeerTotalSupply := totSupplRes.Supply.AmountOf(s.bondDenom)
	xmplTotalSupply := totSupplRes.Supply.AmountOf(s.tokenDenom)

	testcases := []struct {
		name      string
		malleate  func()
		expSupply func(paxeerAddr, xmplAddr common.Address) []bank.Balance
	}{
		{
			"pass - PAXEER and XMPL total supply",
			func() {
				ctx = s.mintAndSendXMPLCoin(ctx, s.keyring.GetAccAddr(0), math.NewInt(1e18))
			},
			func(paxeerAddr, xmplAddr common.Address) []bank.Balance {
				return []bank.Balance{{
					ContractAddress: paxeerAddr,
					Amount:          paxeerTotalSupply.BigInt(),
				}, {
					ContractAddress: xmplAddr,
	...
	for _, tc := range testcases {
		s.Run(tc.name, func() {
			...
			err = s.precompile.UnpackIntoInterface(&balances, method.Name, bz)
			s.Require().NoError(err)
			s.Require().Equal(tc.expSupply(s.paxeerAddr, s.xmplAddr), balances)
		})
	}
}
