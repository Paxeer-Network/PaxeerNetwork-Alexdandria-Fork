...
import (
	...
	"github.com/Paxeer-Network/hyperpaxeer-os/precompiles/werc20"
	"github.com/Paxeer-Network/hyperpaxeer-os/precompiles/werc20/testdata"
...
func TestPrecompileIntegrationTestSuite(t *testing.T) {
	...
	RunSpecs(t, "WHPX precompile test suite")
}
...
var _ = When("a user interact with the WHPX precompiled contract", func() {
	var (
	...
	BeforeEach(func() {
		...
		is.precompileAddrHex = network.GetWHPXContractHex(is.network.GetChainID())

		ctx := integrationNetwork.GetContext()

		// Perform some check before adding the precompile to the suite.

		// Check that WHPX is part of the native precompiles.
		...
		Expect(erc20Params.NativePrecompiles).To(
			...
			"expected whpx to be in the native precompiles",
		)
		...
		// Check that WHPX is registered in the token pairs map.
		...
		Expect(found).To(BeTrue(), "expected whpx precompile to be registered in the tokens map")
		...
		revertCallerContract, err := testdata.LoadWHPX9TestCaller()
	...
	Context("calling an erc20 method", func() {
		...
		When("querying information", func() {
			...
			It("should return the correct name", func() {
				...
				Expect(err).ToNot(HaveOccurred(), "failed to unpack result")
				Expect(name).To(ContainSubstring("HyperPaxeer"), "expected different name")
			})
			...
			It("should return the correct symbol", func() {
				...
				Expect(err).ToNot(HaveOccurred(), "failed to unpack result")
				Expect(symbol).To(ContainSubstring("PAXEER"), "expected different symbol")
			})
