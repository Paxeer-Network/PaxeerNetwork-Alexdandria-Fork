...
import (
	...
	auth "github.com/Paxeer-Network/hyperpaxeer-os/precompiles/authorization"
	"github.com/Paxeer-Network/hyperpaxeer-os/precompiles/erc20"
	"github.com/Paxeer-Network/hyperpaxeer-os/precompiles/erc20/testdata"
	...
	"github.com/Paxeer-Network/hyperpaxeer-os/testutil/integration/os/grpc"
	"github.com/Paxeer-Network/hyperpaxeer-os/testutil/integration/os/keyring"
	"github.com/Paxeer-Network/hyperpaxeer-os/testutil/integration/os/network"
	integrationutils "github.com/Paxeer-Network/hyperpaxeer-os/testutil/integration/os/utils"
	utiltx "github.com/Paxeer-Network/hyperpaxeer-os/testutil/tx"
...
var _ = Describe("ERC20 Extension -", func() {
	...
	BeforeEach(func() {
		...
		Expect(common.HexToAddress(erc20Params.NativePrecompiles[0])).To(Equal(common.HexToAddress(testconstants.WHPXContractMainnet)))
	...
	Context("basic functionality -", func() {
		...
		When("calling reverter contract", func() {
			Context("in a direct call to the WHPX contract", func() {
				var (
				...
				DescribeTable("it should revert token transfer from the WHPX contract", func(before bool, after bool) {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
					amountToSend := big.NewInt(100)
				...
				It("it should send token transfer and send from WHPX contract", func() {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
				...
				DescribeTable("it should revert token transfer and send from WHPX contract", func(before bool, after bool) {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
					balRes, err := is.handler.GetBalanceFromBank(receiver.Bytes(), is.bondDenom)
					Expect(err).To(BeNil())
		...
		When("querying allowance", func() {
			...
			When("querying the allowance for the own address", func() {
				...
				//
				// For reference see this comment: https://github.com/Paxeer-Network/hyperpaxeer-os/pull/2088#discussion_r1407646217
...
var _ = Describe("ERC20 Extension migration Flows -", func() {
	...
	When("using Paxeer (not wPaxeer) in smart contracts", func() {
		It("should be using straight Paxeer for sending funds in smart contracts", func() {
			Skip("will be addressed in follow-up PR")
