...
let
  ...
  ldflags = lib.concatStringsSep "\n" ([
    "-X github.com/cosmos/cosmos-sdk/version.Name=hyperpaxeer"
...
buildGoApplication rec {
  ...
  subPackages = [ "cmd/hyperpaxd" ];
  CGO_ENABLED = "1";

  postFixup = if dbBackend == "rocksdb" then
    ''
      # Rename the binary from hyperpaxd to hyperpaxd-rocksdb
      mv $out/bin/hyperpaxd $out/bin/hyperpaxd-rocksdb
    '' else '''';

  meta = with lib; {
    description = "HyperPaxeer Network is a scalable and interoperable blockchain, built on Proof-of-Stake with fast-finality using the Cosmos SDK which runs on top of CometBFT Core consensus engine.";
    homepage = "https://github.com/paxeer/hyperpaxeer";
    license = licenses.asl20;
    mainProgram = "hyperpaxd";
  };
}
